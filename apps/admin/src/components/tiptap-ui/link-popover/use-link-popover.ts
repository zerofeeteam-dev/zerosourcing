...
export interface UseLinkPopoverConfig {
  ...
  onSetLink?: () => void
  /**
   * Optional URL guard used before a link is written to the editor document.
   * Return null to keep the current document unchanged.
   */
...
export function useLinkHandler(props: LinkHandlerProps) {
  const { editor, normalizeUrl, onSetLink } = props
  ...
  const setLink = useCallback(() => {
    ...
    const normalizedUrl = normalizeUrl ? normalizeUrl(url) : url
    ...
    onSetLink?.()
  }, [editor, normalizeUrl, onSetLink, url])
